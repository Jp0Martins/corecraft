...
async function refreshMempoolIntel() {
  const el = document.getElementById("mempoolIntel");
  const errEl = document.getElementById("mempoolIntelError");
  hideError(errEl);

  try {
    const data = await apiGet("/api/mempool/summary");
    setKV(el, {
      "total txs": data.tx_count,
      "total vsize": data.total_vsize.toLocaleString() + " vB",
      "fee média": data.avg_fee_rate + " sat/vB",
      "fee mín": data.min_fee_rate + " sat/vB",
      "fee máx": data.max_fee_rate + " sat/vB",
      "🟢 low (<10)": data.fee_distribution.low,
      "🟡 medium (10-50)": data.fee_distribution.medium,
      "🔴 high (>50)": data.fee_distribution.high
    });
  } catch (e) {
    showError(errEl, e.message);
  }
...
async function refreshSyncStatus() {
  ...
  try {
    ...
    setKV(el, {
      "blocks": data.blocks,
      "headers": data.headers,
      "lag": data.lag === 0 ? "0 ✅ Sincronizado" : data.lag + " blocos atrás ⚠️"
    });
  } catch (e) {
    showError(errEl, e.message);
  }
